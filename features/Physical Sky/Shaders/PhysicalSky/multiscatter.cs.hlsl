	...
	for (uint i = 0; i < sqrt_samples; ++i)
		for (uint j = 0; j < sqrt_samples; ++j) {
			...
			for (uint step = 1; step <= nsteps; ++step) {
				...
				lum += scatter_integeral * transmittance * phys_sky[0].source_illuminance;
			...
			if (ground_dist > 0) {
				...
				if (dot(pos, sun_dir) > 0) {
					...
					lum += transmittance * phys_sky[0].ground_albedo * tex_transmittance.SampleLevel(samp_linear, samp_coord, 0).rgb * phys_sky[0].source_illuminance;
				}
			}
